...
if (isset($_GET['bid']) && isset($_GET['stu_id'])) {
    ...
    // Fetch renewal book details
    $renew_book_query = "SELECT * FROM int_student_renewal_books WHERE bid='$bid' AND stu_id='$stu_id'";
    ...
    // Insert into student_renewed_books table
    $issue_query = "INSERT INTO student_renewed_books (renewal_id, stu_id, username, rollno, 
    ...
    if (mysqli_query($conn, $issue_query)) {
        ...
        $update_query = "UPDATE issued_books SET return_date = '$new_return_date' WHERE bid='$bid' AND stu_id='$stu_id'";
        mysqli_query($conn, $update_query);

        // Delete from int_student_renewal_books
        $delete_query = "DELETE FROM int_student_renewal_books WHERE bid='$bid' AND stu_id='$stu_id'";
        ...
        try {
            ...
            echo "<script>alert('Book Renewed successfully! & Email Notification Sent Successfully!.'); window.location.href='librarian_renew_students_books.php';</script>";
            exit();
        } catch (Exception $e) {
            echo "<script>alert('Book Renewed successfully, but email could not be sent. Error: " . $mail->ErrorInfo . "'); window.location.href='librarian_renew_students_books.php';</script>";
        }
    } else {
        echo "<script>alert('Error Renewing book: " . mysqli_error($conn) . "'); window.location.href='librarian_renew_students_books.php';</script>";
    }
} else {
    header("Location: librarian_renew_students_books.php");
    exit();
}
?>