...
$query = "SELECT * FROM issued_books WHERE 1=1";
if (!empty($search)) {
    $query .= " AND (accession_number LIKE '%$search%' OR title LIKE '%$search%' OR author LIKE '%$search%' OR username LIKE '%$search%' OR selectyourdegree LIKE '%$search%' OR selectyourcourse LIKE '%$search%')";
}
if (!empty($start_date) && !empty($end_date)) {
    $query .= " AND issued_date BETWEEN '$start_date' AND '$end_date'";
}
...
if (isset($_POST["generate_pdf"])) {
    ...
    // Title
    $pdf->SetXY(10, 10);
    $pdf->Cell(0, 10, 'Library Management System - Issued Books Report', 0, 1, 'C');
    ...
    $header = array('S.NO', 'Acc No', 'Title', 'Author', 'Student Name', 'Degree', 'Course', 'Year', 'Issued Date', 'Due Date');
    $columnWidths = array(10, 15, 50, 40, 30, 25, 30, 10, 25, 25);
    ...
    $pdf_filename = 'Issued_Books_Report_' . date('Y-m-d');
    if (!empty($search)) {
...
<div class="container mt-5">
    <h2 class="text-center mb-4 text-warning">📚 All Issued Books</h2>
    ...
    <table class="table table-bordered table-hover">
        ...
        <tbody id="bookTable">
            ...
            if (mysqli_num_rows($result) > 0) {
                ...
                echo '<tr><td colspan="15" class="no-results">No issued books found matching your criteria</td></tr>';
            }
            ?>
        </tbody>
    </table>
